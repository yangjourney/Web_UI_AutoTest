...
if __name__ == '__main__':
    ...
    markfile(mark=False)
    if len(sys.argv) < 2:
        print ('AutoTest setup with all modules.')
    elif '-m' == sys.argv[1] or 'min'in sys.argv[1]:
        print ('AutoTest setup with the minimum modules.')
        markfile(all_modules)
    elif '-w' == sys.argv[1] or 'without' in sys.argv[1]:
        if not sys.argv[2:]:
            print ('AutoTest setup without the modules:', sys.argv[2:])
